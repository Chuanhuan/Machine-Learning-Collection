...
# |%%--%%| <PsXhzxYQlM|iJb2lX1jRE>

train_df.head()

# |%%--%%| <iJb2lX1jRE|QYPp8Z7JDG>

test_df.head()

# |%%--%%| <QYPp8Z7JDG|pzmrCSckN9>

submission_df.head()

# |%%--%%| <pzmrCSckN9|vdQYABePYO>

train_df, val_df = train_test_split(
    train_df, test_size=0.1, random_state=RANDOM_SEED, shuffle=True
)

# |%%--%%| <vdQYABePYO|FjL0vuPQ89>
...
for epoch in tqdm(range(EPOCHS), position=0, leave=True):
    ...
    print(
        f"Valid Accuracy EPOCH {epoch+1}: {sum(1 for x,y in zip(val_preds, val_labels) if x == y) / len(val_labels):.4f}"
    )
    print("-" * 30)
